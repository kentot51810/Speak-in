...
public final class QueryUtils {
     ...
     */
    public static Word fetchWordData(String requestUrl) {
        // Create URL object
        ...
        Word words = extractFeatureFromJson(jsonResponse);
    ...
    private static String makeHttpRequest(URL url) throws IOException {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Problem retrieving the Word JSON results.", e);
        } finally {
     ...
     */
    private static Word extractFeatureFromJson(String WordJSON) {
        ...
        List<String> shortDefinition = new ArrayList<>();
        ...
        try {
            ...
            JSONArray baseJsonResponse = new JSONArray(WordJSON);

            // For each Word in the WordArray, create an {@link Word} object
            for (int i = 0; i < baseJsonResponse.length(); i++) {

                // Get a single Word at position i within the list of words
                JSONObject currentWord = baseJsonResponse.getJSONObject(i);

                // For a given Word, extract the JSONObject associated with the
                // key called "properties", which represents a list of all properties
                // for that Word.
                JSONArray shorDefArray = currentWord.getJSONArray("shortdef");

                for (int j = 0; j < shorDefArray.length(); j++){
                    String shortDefinitionResult = shorDefArray.getString(j);
                    shortDefinition.add(shortDefinitionResult);

                }

                word.setShortDefinition(shortDefinition);
